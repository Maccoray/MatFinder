...
@CommandName("FinderFragments")
@Category("Finder")
@Name("10|Fragments")
@Help("Find Fragments")

public class FragmentsQuery implements IQuery {

    @Argument
	public ISnapshot mOpenSnapshot;
	
	@Argument(isMandatory = false, flag = "b")
	@Help("Base Fragment class Name")
	public String mStringBaseClassName = "android.support.v4.app.Fragment";
	
	Logger mLog = Logger.getLogger(ActivityQuery.class.toString());
	
	public FragmentsQuery() {
	...
	public IResult execute(IProgressListener listener) throws Exception
	{
			...
			while(classSuperClass.hasSuperClass()){
				...
				if( stringSuperClassName.equals(mStringBaseClassName) ){
					mLog.info("Find SuperClass !"+mStringBaseClassName+
							" FragmentName:" + classfint.getName());
		...
		ReportManager.ReportMsg msg = new ReportManager.ReportMsg(ReportManager.FINDER_RPMSG_Fragments); 
